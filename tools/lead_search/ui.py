...
# Page config
st.set_page_config(
    page_title="Lead Search Agent - Automation Tools Hub",
    page_icon="🔍",
    layout="wide"
)

def main():
    st.title("🔍 Lead Search Agent")
    st.markdown("Validate business contacts using AI-powered employment verification.")
    ...
    with col1:
        st.markdown("""
        **How it works:**
        1. Upload CSV with contact details (eC_ID, Company_Name, Full_Name, Title, Company_Website)
        2. File is processed in GCP using AI analysis and Google search
        3. Download results with verification status and evidence
    ...
    with col2:
        ...
        **Output columns:**
        ...
        - ai_note: AI reasoning
        - confidence_level: High/Medium/Low
        - matched_*: Alternative matches if not verified
        """)
    ...
    uploaded_file = st.file_uploader(
        "Choose CSV file",
        type=['csv'],
        help="Upload CSV with eC_ID, Company_Name, Full_Name, Title, Company_Website columns"
    )
    ...
    if uploaded_file:
        ...
        try:
            ...
            st.success(f"✅ File loaded: {len(df)} contacts")
            
            # Validate columns
            required_cols = ['eC_ID', 'Company_Name', 'Full_Name', 'Title']
    ...
    if 'current_session_id' in st.session_state:
        ...
        display_session_results(st.session_state.current_session_id)
...
def process_large_dataset_chunked(df, uploaded_file, chunk_size):
    ...
    try:
        ...
        for chunk_idx in range(total_chunks):
            ...
            status_text.text(f"Processing chunk {chunk_idx + 1}/{total_chunks} (contacts {start_idx + 1}-{end_idx})")
...
def download_results(job_id: str, bucket_manager):
    ...
    try:
        ...
        if results_data:
            ...
            st.success(f"✅ Results downloaded: {len(df_results)} contacts validated")
            
            # Display results summary
            st.subheader("📊 Validation Summary")
            
            # Count verification statuses
            verified_count = len(df_results[df_results['verified?'] == 'Yes'])
            not_verified_count = len(df_results[df_results['verified?'] == 'No'])
            inconclusive_count = len(df_results[df_results['verified?'] == 'Inconclusive'])
            
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Total Contacts", len(df_results))
            with col2:
                st.metric("✅ Verified", verified_count)
            with col3:
                st.metric("❌ Not Verified", not_verified_count)
            with col4:
                st.metric("⚠️ Inconclusive", inconclusive_count)
            ...
            st.download_button(
                "💾 Save Results CSV",
                data=csv_data,
                file_name=f"lead_search_results_{job_id}.csv",
                mime="text/csv"
            )
...
def check_session_status(job_id: str):
    ...
    try:
        ...
        if current_status == 'pending':
            ...
            st.sidebar.info("🔄 AI contact validation in progress...")
            st.sidebar.markdown("**Message:** " + status.get('message', 'Processing...'))
            
        elif current_status == 'completed':
            st.sidebar.success("✅ Contact validation completed! Use the download button below to get results.")
...
def display_session_results(job_id: str):
